...
class NegativePosition(commands.CommandError):
    pass
...
class QueueMenu(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        embed = discord.Embed(title=f'{len(self.data)} songs in the queue', colour=discord.Color.blurple())
    ...
    async def callback(self, interaction: discord.Interaction):
        for track in self.info:
            if self.values[0] == track['info']['identifier']:
                ...
                await self.ctx.send(embed=embed, reply = False)
...
class LoopMenu(discord.ui.Select):
    def __init__(self, bot, ctx):
        ...
        if not self.player.loop == 1:
            values.append(discord.SelectOption(label='Track',description = 'Starts looping your currently playing track.', emoji=f'🔂'))
        if not self.player.loop == 2:
            values.append(discord.SelectOption(label='Queue',description = 'Starts looping your current queue.', emoji=f'🔁'))
        values.append(
            discord.SelectOption(label='Off',description = 'Stops looping.', emoji=f'➡️'
        ))
        values.append(
            discord.SelectOption(label='Cancel',description = 'Cancels the action.', emoji=f'<:tickNo:885222934036226068>'
        ))
...
class Music(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
            ...
            bot.lavalink.add_node('194.163.159.4', 5069, 'panda is the best password', 'eu', 'default-node', 5)  # Host, Port, Password, Region, Name
    ...
    async def ensure_voice(self, ctx):
        ...
        if not player.is_connected:
            ...
            if int(player.channel_id) != ctx.author.voice.channel.id:
                ...
                embed.description = f'{ctx.author.mention}, you must be in {channel.mention} for this session.'
    ...
    async def track_hook(self, event):
        if isinstance(event, TrackStartEvent):
            if event.player.loop != 1:
                ...
                embed.description = f'**[{event.track.title.upper()}]({event.track.uri})**' 
    ...
    def is_privileged(self, ctx: commands.Context):
        ...
        #if len(ctx.author.voice.channel) == 1:return True Already giving the dj perms if the original dj disconnects 
    ...
    async def pause_command(self, ctx: commands.Context):
        ...
        embed = discord.Embed(color = discord.Color.blurple())
        embed.description = "The playback was paused."
        return await ctx.send(embed=embed)
    ...
    async def resume_command(self, ctx: commands.Context):
        ...
        embed = discord.Embed(color = discord.Color.blurple())
        embed.description = "The playback was resumed."
        return await ctx.send(embed=embed)
    ...
    async def stop_command(self, ctx: commands.Context):
        ...
        await player.stop()
        embed = discord.Embed(color = discord.Color.blurple())
        embed.description = "The playback was stopped."
        return await ctx.send(embed=embed)
    ...
    async def clear_command(self, ctx: commands.Context):
        ...
        player.queue.clear()
        embed = discord.Embed(color = discord.Color.blurple())
        embed.description = "Playback stopped."
        return await ctx.send(embed=embed)
    ...
    async def volume_command(self, ctx: commands.Context, volume: int = None):
        """Sets the player's volume; If there is not input, it will show the current value"""
        player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        if not player.is_connected:
            raise NoVoiceChannel
        if volume is None:
        ...
        embed = discord.Embed(colour = discord.Color.blurple(),description = f'The volume was set to **{volume}%**.')
        return await ctx.send(embed=embed)

    @commands.command(name="remove")
    async def remove_range_command(self,ctx: commands.Context,start: int,*,end: int= None):
        """Removes all the tracks from the specified start through the specified end (if end is not specified it will remove only one track - the one with the specified start)"""
        player = self.bot.lavalink.player_manager.get(ctx.guild.id)
        if not player.is_connected:
            raise NoVoiceChannel
        if start < 0:
        ...
        if end == None:
            del queue[start-1]
            embed = discord.Embed(color = discord.Color.blurple(),description=f'Successfully removed track with index `{start}` in the queue.')
        else:
            del queue[start-1:end]
            embed = discord.Embed(color = discord.Color.blurple(),description=f'Successfully removed tracks with index from `{start}` to `{end}` in the queue..')
        player.queue = queue
